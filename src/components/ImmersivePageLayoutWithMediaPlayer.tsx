...
interface ImmersivePageLayoutWithMediaPlayerProps {
  children: ReactNode;
...
/**
 * Updated ImmersivePageLayout using the unified MediaPlayer system
 * 
 * MIGRATION EXAMPLE:
 * This shows how to update existing components to use the unified MediaPlayer
 * instead of importing Player directly.
 */
export const ImmersivePageLayoutWithMediaPlayer: React.FC<ImmersivePageLayoutWithMediaPlayerProps> = ({
  children,
  title,
  subtitle,
  description,
  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 w-full h-screen flex items-center justify-center z-0">
        ...
        {/* Background Video - UPDATED TO USE UNIFIED MEDIAPLAYER */}
        {theme.video && theme.video.url && (
          <>
            <MediaPlayer
              ...
              controls={false} // Background videos shouldn't have controls
              onLoad={() => console.log('Background video loaded')}
              onError={(error: Error) => console.error('Background video error:', error)}
              style={{
...
export default ImmersivePageLayoutWithMediaPlayer;