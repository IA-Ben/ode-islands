...
import { AtHomeARCard, type AtHomeARCardData } from "./AtHomeARCard";
import { PhotoDropCard, type PhotoDropCardData } from "./PhotoDropCard";
import { SmartVideoEditorCard, type SmartVideoEditorCardData } from "./SmartVideoEditorCard";
import { MerchCard, type MerchCardData } from "./MerchCard";
import { NPSReviewCard, type NPSReviewCardData } from "./NPSReviewCard";
...
export function AfterCard({ card, onAction }: AfterCardProps) {
  ...
  switch (card.type) {
      ...
      return (
        <AtHomeARCard
          data={{
            ...
            unlocked: card.metadata?.unlocked ?? true,
            requiredTier: card.metadata?.requiredTier,
            previewImageUrl: card.imageUrl,
            arExperienceId: card.metadata?.arExperienceId,
          }}
      ...
      return (
        <PhotoDropCard
          data={{
            ...
            title: card.title,
            description: card.description || "",
            photoCount: card.metadata?.photoCount || 0,
            newPhotos: card.metadata?.newPhotos,
            collectionUrl: card.metadata?.collectionUrl,
            thumbnails: card.metadata?.thumbnails,
          }}
          onViewGallery={handleAction}
          onDownloadPhotos={() => {
            if (onAction) {
              onAction("download_photos", card);
            }
          }}
          onShareGallery={() => {
            if (onAction) {
              onAction("share_gallery", card);
            }
          }}
        />
      );
    
    case "smart-video-editor":
      return (
        <SmartVideoEditorCard
          data={{
            id: card.id,
            title: card.title,
            description: card.description || "",
            features: card.metadata?.features || [],
            aiPowered: card.metadata?.aiPowered ?? true,
            clipCount: card.metadata?.clipCount,
            previewVideoUrl: card.metadata?.previewVideoUrl,
          }}
          onOpenEditor={handleAction}
        />
      );
      ...
      return (
        <MerchCard
          data={{
            ...
            discountPercentage: card.metadata?.discountPercentage,
            originalPrice: card.metadata?.originalPrice,
            salePrice: card.metadata?.salePrice,
            limitedTime: card.metadata?.limitedTime,
            stockStatus: card.metadata?.stockStatus || "in_stock",
          }}
          onShopNow={handleAction}
        />
      );
      ...
      return (
        <NPSReviewCard
          data={{
            ...
            title: card.title,
            description: card.description || "",
            surveyCompleted: card.metadata?.surveyCompleted,
            incentive: card.metadata?.incentive,
            pointsReward: card.metadata?.pointsReward,
          }}
          onSubmitReview={(rating, feedback) => {
            console.log("Review submitted:", { rating, feedback });
            if (onAction) {
              onAction("submit_review", { ...card, metadata: { ...card.metadata, rating, feedback } });
            }
          }}
        />
      );
    ...
    default:
      // Generic card fallback for all other types
