...
export async function GET(request: NextRequest) {
  try {
    ...
    const lane = searchParams.get('lane'); // 'recap', 'create', 'offers'
    ...
    const conditions: any[] = [
      eq(cards.scope, 'event'), // After phase uses 'event' scope
    ...
    if (lane) {
      conditions.push(
        and(
          eq(cardAssignments.parentType, 'after_lane'),
    ...
    console.error('Error fetching after cards:', error);
    return NextResponse.json(
      { error: 'Failed to fetch after cards' },
      { status: 500 }
    );
  }
}
